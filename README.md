...
# How the code works
The file montecarlo.hpp contains the core of the simulation, which relies on the "code/randnumgen.hpp" library to generate pseudo-random numbers. The "code/main.cpp" file calls these components to execute the program. Finally, an example configuration file named "code/config.txt" is included, from which the program reads the system control parameters.
